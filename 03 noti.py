...
Unit = StringVar()
Inbox = StringVar(value="Inbox")  # Set default value to "Inbox"
SearchText = StringVar()  # Variable to hold the search query
Subject = StringVar()  # Subject for the composed message
AttachmentPath = StringVar()  # Path of the attached file
SendToAll = BooleanVar()  # Checkbox to send to all units
message_records = []  # List to store message records
...
def send_message():
    ...
    messagebox.showinfo("Message Sent", summary)
    compose_win.destroy()  # Close the compose window after sending

# Function to update the message display area
def update_message_display():
    # Clear previous messages
    for widget in message_display_frame.winfo_children():
        widget.destroy()

    # Show each record in the message display area
    for record in message_records:
        record_text = f"To: {record[0]}, Subject: {record[1]}, Message: {record[2]}, Attachment: {record[3] if record[3] else 'None'}"
        record_label = Label(message_display_frame, text=record_text, bg="#D3D3D3", font=("times new roman", 12))
...
message_canvas = Canvas(message_display_frame, bg="#D3D3D3")
message_canvas.pack(side=LEFT, fill=BOTH, expand=True)

# Scrollbar for the message display
scrollbar = Scrollbar(message_display_frame, orient=VERTICAL, command=message_canvas.yview)
scrollbar.pack(side=RIGHT, fill=Y)

message_canvas.configure(yscrollcommand=scrollbar.set)

# Frame inside the canvas to hold the message records
message_frame = Frame(message_canvas, bg="#D3D3D3")
message_canvas.create_window((0, 0), window=message_frame, anchor="nw")
...
def update_message_display():
    ...
    for record in message_records:
        record_text = f"To: {record[0]}, Subject: {record[1]}, Message: {record[2]}, Attachment: {record[3] if record[3] else 'None'}"
        record_label = Label(message_frame, text=record_text, bg="#D3D3D3", font=("times new roman", 12))
        record_label.pack(anchor="w")

    # Update the scroll region of the canvas
    message_frame.update_idletasks()  # Ensure the frame size is updated
    message_canvas.config(scrollregion=message_canvas.bbox("all"))  # Update the scroll region
