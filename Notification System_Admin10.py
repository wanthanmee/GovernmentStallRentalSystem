...
PostCode = tk.StringVar()  # New variable for Post code search
...
def compose_message():
	...
	send_all_cb = tk.Checkbutton(
		...
		font=("helvetica", 12),
		bg="#D3D3D3",
		command=lambda: toggle_postcode_unit_fields(SendToAll.get(), postcode_entry, unit_combo, note_label)
	)
	...
	postcode_entry = tk.Entry(main_frame, textvariable=PostCode, font=("helvetica", 16))
	postcode_entry.grid(row=1, column=1, padx=10, pady=10, sticky="ew")
	PostCode.trace_add("write", filter_units_by_postcode)

	# Stall Unit Combobox
	tk.Label(main_frame, text="Stall Unit", font=("helvetica", 16), bg="#D3D3D3").grid(
		row=2, column=0, padx=10, pady=10, sticky="w")
	unit_combo = ttk.Combobox(main_frame, textvariable=Unit, font=("helvetica", 16), width=40)
	unit_combo.grid(row=2, column=1, padx=10, pady=10, sticky="ew")
	...
	send_button = tk.Button(main_frame, text="Send", font=("helvetica", 16),
	                        bg="#ff8210", fg="white",
	                        command=lambda: modified_send_message(SendToAll.get()))
...
def modified_send_message(send_to_all):
	...
	# Determine recipients based on send_to_all flag
	recipients = []
	if send_to_all:
		# Get all units from all postcodes
		for postcode in stall_database:
			recipients.extend(stall_database[postcode])
	else:
		postcode = PostCode.get().strip()
		unit = Unit.get().strip()

		if not postcode:
			messagebox.showwarning("Missing Information", "Please enter a postcode!")
			return
		if postcode not in stall_database:
			messagebox.showwarning("Invalid Postcode", "Please enter a valid postcode!")
			return

		if unit:  # If a specific unit is selected
			if unit not in stall_database[postcode]:
				messagebox.showwarning("Invalid Unit", "Selected unit is not associated with this postcode!")
				return
			recipients = [unit]
		else:  # If no unit is selected, send to all units in the postcode
			recipients = stall_database[postcode]
	...
	# Confirm if sending to many recipients
	if len(recipients) > 10:  # You can adjust this threshold
		if not messagebox.askyesno("Confirm",
		                           f"You are about to send this message to {len(recipients)} units. Are you sure?"):
			return

	# Send message to all recipients
	success = True
	for recipient in recipients:
		if not insert_message_to_tables(
				current_user,
				recipient,
				subject,
				message,
	...
	if success:
		update_message_display()
		messagebox.showinfo("Success", f"Message sent successfully to {len(recipients)} unit(s)!")
		compose_win.destroy()
	else:
		messagebox.showerror("Error", "Failed to send message")
...
tk.Label(entries_frame, text="Admin - INBOX", font=("helvetica", 30, "bold"), bg="#F5F5F5").grid(row=0, column=0,
                                                                                                 columnspan=7,
                                                                                                 padx=10, pady=20,
                                                                                                 sticky="nsew")
...
tk.Label(entries_frame, text="Search Unit:", font=("helvetica", 16), bg="#F5F5F5").grid(row=1, column=4, padx=10,
                                                                                        pady=10, sticky="nsew")
tk.Entry(entries_frame, textvariable=SearchText, font=("helvetica", 16), width=30).grid(row=1, column=5, padx=10,
                                                                                        pady=10, sticky="nsew")
tk.Button(entries_frame, text="Search", font=("helvetica", 16), fg="white", bg="#ff8210", command=search_units).grid(
	row=1, column=6, padx=10, pady=10, sticky="nsew")
...
tk.Button(button_frame, text="Compose Message", font=("helvetica", 16), bg="#ff8210", fg="white",
          command=compose_message).grid(row=0, column=0, padx=5, pady=5, sticky="ew")
tk.Button(button_frame, text="Delete Message", font=("helvetica", 16), bg="#ff8210", fg="white",
          command=delete_message).grid(row=0, column=1, padx=5, pady=5, sticky="ew")
