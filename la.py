...
def get_messages(user, category):
    ...
    try:
        ...
        elif category == "Inbox":  # New messages
            query = """
                SELECT message_id, sender, recipient, subject, message, attachment, timestamp_receive, status
                FROM notif_inbox 
                WHERE recipient = ? 
        ...
        messages = c.fetchall()
        if not messages:
            print(f"No messages found for user: {user} in category: {category}")
        return messages
...
def update_message_display(tenant_id_filter=None):
    ...
    messages = get_messages(current_user, selected_category)  # Get messages based on current user and selected category
